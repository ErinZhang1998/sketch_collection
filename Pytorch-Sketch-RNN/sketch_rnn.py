...
class HParams():
    def __init__(self):
        self.data_location = '/raid/xiaoyuz1/sketch_datasets/spg_body/body.npz'
...
def purify(strokes):
    ...
    data = []
    kept_idx = np.ones(len(strokes)).astype(bool)
    for seq_idx, seq in enumerate(strokes):
...
def make_batch(batch_size, random=True, selected_idx=[]):
    ...
    if not random and len(selected_idx) > 0:
...
class Model():
    ...
    def save(self, epoch):
        ...
        torch.save(self.encoder.state_dict(), \
            '/raid/xiaoyuz1/sketch_datasets/spg_body/model/encoderRNN_sel_%3f_epoch_%d.pth' % (sel,epoch))
        torch.save(self.decoder.state_dict(), \
            '/raid/xiaoyuz1/sketch_datasets/spg_body/model/decoderRNN_sel_%3f_epoch_%d.pth' % (sel,epoch))
    ...
    def condition_generation_one(self, epoch, batch, lengths, save_name):
        ...
        make_image(batch,lengths, sequence, epoch, name=save_name)
    
    def conditional_generation(self, epoch):
        for gen_idx in range(10):
            batch, lengths = make_batch(1)
            name='_output_{}_'.format(gen_idx)
            self.condition_generation_one(epoch, batch, lengths, '_output_{}_'.format(gen_idx))
...
def make_image(batch, lengths, sequence, epoch, name=None):
    """plot drawing with separated strokes"""
    
    batch = batch.squeeze(1).cpu().numpy()
    lengths = lengths[0]
    batch_copy = np.copy(batch)
    assert len(batch_copy.shape) == 2
    batch_copy = denormalize([batch_copy], scale_factor)[0]

    batch_copy[:, 0] = np.cumsum(batch_copy[:,0], 0)
    batch_copy[:, 1] = np.cumsum(batch_copy[:,1], 0)
    batch_copy = batch_copy[:lengths]
    strokes_batch = np.split(batch_copy, np.where(batch_copy[:, 2] < 1)[0] + 1)
    fig = plt.figure(figsize=(5,10))

    ax1 = fig.add_subplot(311)
    for s in strokes_batch:
    ...
    sequence  = denormalize([sequence], scale_factor)[0]
    strokes = np.split(sequence, np.where(sequence[:, 2] > 0)[0] + 1)
    ...
    pil_image.save(final_name,"JPEG")
    plt.close("all")
