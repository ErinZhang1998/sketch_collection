...
class HParams(object):
    def __init__(self,
        ...
        self.num_steps=num_steps# Total number of steps of training. Keep large.
        self.save_every=2000# Number of batches per checkpoint creation.
        self.img_H=48
        self.img_W=48
        self.max_seq_len=250# Not used. Will be changed by model. [Eliminate?]
        self.dec_rnn_size=512# Size of decoder.
        self.dec_model='hyper'# Decoder: lstm, layer_norm or hyper.
        self.z_size=128# Size of latent vector z. Recommend 32, 64 or 128.
        self.batch_size=100# Minibatch size. Recommend leaving at 100.
        self.grad_clip=1.0# Gradient clipping. Recommend leaving at 1.0.
        self.num_mixture=20# Number of mixtures in Gaussian mixture model.
        self.learning_rate=0.001# Learning rate.
        self.decay_rate=0.9999# Learning rate decay per minibatch.
        self.min_learning_rate=0.00001# Minimum learning rate.
        self.use_recurrent_dropout=True# Dropout with memory loss. Recommended
...
def copy_hparams(hparams):
    ...
    return HParams(data_set=hparams.data_set,  # Our dataset.
        num_steps=hparams.num_steps,  # Total number of steps of training. Keep large.
        save_every=hparams.save_every,  # Number of batches per checkpoint creation.
        img_H=hparams.img_H,
        img_W=hparams.img_W,
        max_seq_len=hparams.max_seq_len,  # Not used. Will be changed by model. [Eliminate?]
        dec_rnn_size=hparams.dec_rnn_size,  # Size of decoder.
        dec_model=hparams.dec_model,  # Decoder: lstm, layer_norm or hyper.
        z_size=hparams.z_size,  # Size of latent vector z. Recommend 32, 64 or 128.
        batch_size=hparams.batch_size,  # Minibatch size. Recommend leaving at 100.
        grad_clip=hparams.grad_clip,  # Gradient clipping. Recommend leaving at 1.0.
        num_mixture=hparams.num_mixture,  # Number of mixtures in Gaussian mixture model.
        learning_rate=hparams.learning_rate,  # Learning rate.
        decay_rate=hparams.decay_rate,  # Learning rate decay per minibatch.
        min_learning_rate=hparams.min_learning_rate,  # Minimum learning rate.
        use_recurrent_dropout=hparams.use_recurrent_dropout,  # Dropout with memory loss. Recommended
        recurrent_dropout_prob=hparams.recurrent_dropout_prob,  # Probability of recurrent dropout keep.
        use_input_dropout=hparams.use_input_dropout,  # Input dropout. Recommend leaving False.
        input_dropout_prob=hparams.input_dropout_prob,  # Probability of input dropout keep.
        use_output_dropout=hparams.use_output_dropout,  # Output dropout. Recommend leaving False.
        output_dropout_prob=hparams.output_dropout_prob,  # Probability of output dropout keep.
        random_scale_factor=hparams.random_scale_factor,  # Random scaling data augmentation proportion.
        augment_stroke_prob=hparams.augment_stroke_prob,  # Point dropping augmentation proportion.
        conditional=hparams.conditional,  # When False, use unconditional decoder-only model.
        is_training=hparams.is_training,  # Is model training? Recommend keeping true.)
    )
